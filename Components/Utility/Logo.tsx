...
type LogoSize = 'defaultLogoSize' | 'headerLogoSize';
type LogoColoring = 'defaultLogoColoring' | 'whiteLogoColoring';
...
const Logo: React.FC<LogoProps> = ({ size="defaultLogoSize", coloring="defaultLogoColoring", clickable=true }) => {
    const handlePress = () => {
        if (clickable) {
            router.push("/");
        }
    ...
    return (
        <TouchableOpacity onPress={handlePress} style={[styles.logoContainer, styles[size] as any ]}>
            <Svg width="16" height="18" viewBox="0 0 16 18" fill="none" style={[styles[`${size}Image`] as any]}>
                <G clipPath="url(#clip0_1216_4114)">
            ...
            <AppText style={[styles.logoContainerText, styles[coloring] as any]}>CLUBBERA</AppText>
        </TouchableOpacity>
    );
};
